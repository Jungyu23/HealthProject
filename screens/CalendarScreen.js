...
const CalendarScreen = () => {
  ...
  const [workoutType, setWorkoutType] = useState('전신'); // 초기 운동 유형 설정
  const [selectedWorkout, setSelectedWorkout] = useState(workoutTypes['전신'][0]); // 초기 선택된 운동 설정
  ...
  useEffect(() => {
    setMarkedDates(getMarkedDates(workouts));
  }, [workouts]);
  ...
  return (
    <View style={styles.container}>
      <Calendar
        ...
        monthFormat={'yyyy MM'}
        markedDates={getMarkedDates(workouts)}
      ...
      >
        <View style={styles.modalView}>
          {/* 운동 종류와 구체적인 운동을 선택하는 RNPickerSelect */}
          <Text>운동 종류 선택:</Text>
          <RNPickerSelect
            onValueChange={(itemValue) => {
             setWorkoutType(itemValue);
             setSelectedWorkout(workoutTypes[itemValue][0]);
            }}
            items={Object.keys(workoutTypes).map((type) => ({
              label: type,
              value: type,
            }))}
          ...
          <RNPickerSelect
            ...
            items={workoutTypes[workoutType].map((workout) => ({
              label: workout,
              value: workout,
            }))}
            style={pickerSelectStyles}
